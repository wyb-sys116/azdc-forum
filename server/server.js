...
if (!process.env.DATABASE_PRIVATE_URL) {
  console.error('❌ 未找到 DATABASE_PRIVATE_URL 环境变量，请确认 PostgreSQL 已通过私有网络关联到当前服务');
}
...
async function initDB() {
  try {
    ...
    if (parseInt(rows[0].count) === 0) {
      ...
      console.log('✅ 默认示例数据已插入');
    }

    console.log('✅ 数据库初始化完成，连接成功');
  } catch (err) {
...
app.get('/api/posts', async (req, res) => {
  try {
    ...
    if (keyword) {
      sql += ` AND (LOWER(title) LIKE LOWER($${idx}) OR LOWER(content) LIKE LOWER($${idx}) OR LOWER(author) LIKE LOWER($${idx}))`;
      params.push(`%${keyword}%`);
      idx++;
    }

    if (sort === 'hot') sql += ' ORDER BY likes DESC';
    else if (sort === 'views') sql += ' ORDER BY views DESC';
